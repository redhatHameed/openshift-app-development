...
oc set volume dc/${APP} --add --name=${APP}-config --mount-path=/opt/eap/standalone/configuration/app-config --sub-path=mlbparks.json --configmap-name=${APP}-config -n ${DEV_PROJECT}
