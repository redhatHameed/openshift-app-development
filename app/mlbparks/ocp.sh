...
oc create configmap ${APP}-config --from-file=config/mlbparks.json -n ${DEV_PROJECT}
oc set volume dc/${APP} --add --name=${APP}-config --mount-path=/opt/eap/standalone/configuration/mlbparks.json --sub-path=mlbparks.json --configmap-name=${APP}-config -n ${DEV_PROJECT}
